...
export function InventorySelectorModal({ onClose, onConfirm }: InventorySelectorModalProps) {
    const { user } = useAuth();
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-5xl w-full h-[90vh] shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                    <div>
                        ...
                        <p className="text-slate-500 mt-1">Añade productos del inventario al encargo. El stock se descontará al guardar el encargo.</p>
                    </div>
                    <button type="button" onClick={onClose} className="text-slate-400 hover:text-rose-500 transition-colors p-2 rounded-full hover:bg-rose-50 dark:hover:bg-rose-900/20">
                        <X size={24} />
                    </button>
                </div>
                ...
                <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                    ...
                    <div className="w-full md:w-96 bg-slate-50 dark:bg-slate-900/50 flex flex-col h-[40vh] md:h-auto border-t md:border-t-0 border-slate-200 dark:border-slate-800">
                        <div className="p-4 border-b border-slate-200 dark:border-slate-800 font-black text-slate-800 dark:text-white flex items-center gap-2">
                            <ShoppingCart size={20} /> Productos Seleccionados
                        </div>
                        <div className="flex-1 overflow-y-auto p-4 space-y-3">
                            {cart.length === 0 ? (
                                    ...
                                    <div key={c.item.id} className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col gap-2">
                                        ...
                                        <div className="flex justify-between items-center">
                                            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900 rounded-lg p-1">
                                                <button type="button" onClick={() => updateQuantity(c.item.id, -1)} className="p-1 hover:bg-white dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300">
                                                    <Minus size={14} />
                                                </button>
                                            ...
                                            <button type="button" onClick={() => removeFromCart(c.item.id)} className="text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 p-1.5 rounded-lg">
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div className="p-6 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700">
                            <div className="flex justify-between items-center mb-4 text-lg">
                                <span className="font-bold text-slate-600 dark:text-slate-400">Total seleccionado:</span>
                            ...
                            >
                                <ShoppingCart size={20} /> Añadir al Formulario
                            </button>
                        </div>
                    </div>
                </div>
